...
→ https://play.linkanddink.com/#pd-survey
---`;
...
function youthTemplate(name: string, _includeLnd: boolean): string {
  return `Hi ${name},
...
Would you be open to a quick call this week or next? Just reply with a couple of times that work and we'll find something.${includeLnd ? `\n\nFor context, I run a local pickleball community called Link & Dink with 900+ players across two Dill Dinkers locations in Montgomery County: https://play.linkanddink.com` : ""}
